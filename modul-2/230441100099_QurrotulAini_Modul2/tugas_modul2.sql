...
ALTER TABLE umkm
ADD CONSTRAINT fk_umkm_skala
    FOREIGN KEY (id_skala) REFERENCES skala_umkm(id_skala);

ALTER TABLE umkm
ADD CONSTRAINT fk_umkm_kabupaten
...
SELECT
    ...
    (SELECT p.nama_lengkap 
     FROM pemilik_umkm p 
     WHERE p.id_pemilik = u.id_pemilik) AS nama_pemilik,
    (SELECT k.nama_kategori 
     FROM kategori_umkm k 
     WHERE k.id_kategori = u.id_kategori) AS kategori_umkm,
    (SELECT s.nama_skala 
     FROM skala_umkm s 
     WHERE s.id_skala = u.id_skala) AS skala_usaha,
    (SELECT kk.nama_kabupaten_kota 
     FROM kabupaten_kota kk 
     WHERE kk.id_kabupaten_kota = u.id_kabupaten_kota) AS nama_kabupaten_kota,
    u.tahun_berdiri
FROM
    umkm u;
...
SELECT 
    ...
    u.nama_usaha
FROM 
    pemilik_umkm p, umkm u
WHERE 
    p.id_pemilik = u.id_pemilik;
...
SELECT 
    ...
    u.omzet_per_tahun
FROM 
    umkm u, pemilik_umkm p, skala_umkm s
WHERE 
    u.id_pemilik = p.id_pemilik
    AND u.id_skala = s.id_skala
    AND s.nama_skala = 'Menengah';
...
FROM 
    umkm u, kabupaten_kota kk
WHERE 
    u.id_kabupaten_kota = kk.id_kabupaten_kota
GROUP BY 
    kk.nama_kabupaten_kota;
